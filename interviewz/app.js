...
// Cache key
const CSV_CACHE_KEY = 'talent_tracker_csv_cache';

// Empty-state display constants
const EMPTY_VALUE = '-';
const EMPTY_PROSE = 'Not available.';
...
class FormApp {
  ...
  initDoms() {
    ...
    // Cache section reference to avoid querying on every keydown
...
const linkJobUrlAnchor = document.getElementById('linkJobUrlAnchor');
...
function setupEventListeners() {
  ...
  // Interview Company Notes Submit & Reset
  ...
  fetch(SHEET_EXPORT_URL)
    ...
    .then(csvText => {
      try {
        ...
        localStorage.setItem(CSV_CACHE_KEY, JSON.stringify(cacheObj));
      } catch (e) {
      ...
      if (csvText !== cachedCsvText) {
        ...
        console.log('[TalentTracker] Fetched data is identical to cache, skipping re-render');
      }
...
function parseAndInitializeData(csvText) {
  ...
  // Resolves 'Job Suitability' (space) → 'Job_Suitability' (underscore) everywhere downstream
...
function applyFilters(resetPage = true) {
  ...
  renderTable();
  if (typeof rawApplications !== 'undefined' && rawApplications.length > 0) {
...
function renderTable() {
  ...
  registryTableBody.appendChild(frag);

  // Update pagination info and state
  if (paginationInfo) {
    paginationInfo.textContent = `Showing ${totalRows === 0 ? 0 : startIdx + 1} to ${endIdx} of ${totalRows} applications`;
  }
...
function openDetailsDrawer(app) {
  ...
  if (drawerCompanyNameDisplay) drawerCompanyNameDisplay.textContent = companyVal;
  if (drawerJobTitle)          drawerJobTitle.value                = jobTitleVal;
  if (drawerCompanyName)        drawerCompanyName.value              = companyVal;
  ...
  if (hiringTeamVal) {
    ...
    drawerHiringTeam.textContent = EMPTY_VALUE;
  }
  ...
  if (followUpVal) {
    ...
    drawerFollowUp.textContent = EMPTY_VALUE;
  }
  ...
  // Comments — rendered as markdown for consistent rich-text formatting
  const comments = (app['Comments'] || '').trim();
  drawerComments.innerHTML = comments ? parseMarkdown(comments) : EMPTY_VALUE;

  // Job and Company Descriptions
  drawerJobDescription.textContent = (app['Job Description'] || EMPTY_PROSE).trim();
  drawerCompanyDescription.textContent = (app['Company Description'] || EMPTY_PROSE).trim();

  // Links
  const jobUrl = (app['Job URL'] || '').trim();
  if (linkJobUrl) linkJobUrl.value = jobUrl;
  if (linkJobUrlAnchor) {
    if (jobUrl) { linkJobUrlAnchor.href = jobUrl; linkJobUrlAnchor.style.display = ''; }
    else        { linkJobUrlAnchor.style.display = 'none'; }
  }
  ...
  if (hasInterview) {
    ...
    // Populate notes from spreadsheet data
 ...
 * Call once per render cycle and pass the result to chart functions.
 */
...
function initCumulativeSubmissionsChart(applications, tokens) {
  ...
  if (cumulativeSubmissionsChartInstance) cumulativeSubmissionsChartInstance.destroy();
...
function initStatusSplitChart(applications, tokens) {
  ...
  if (statusSplitChartInstance) statusSplitChartInstance.destroy();
...
function initSuitabilityBarChart(applications, tokens) {
  ...
  if (suitabilityBarChartInstance) suitabilityBarChartInstance.destroy();
...
function initTopCompaniesChart(applications, tokens) {
  ...
  if (topCompaniesChartInstance) topCompaniesChartInstance.destroy();
...
function renderActiveInterviewsPanel(applications) {
  ...
  if (interviewApps.length === 0 || !isHomeTab) {
    if (isHomeTab && hasActiveFilters) {
      ...
      if (activeInterviewsCountEl) activeInterviewsCountEl.textContent = '0';
      gridEl.innerHTML = '<div class="no-results-pipeline">No results found for this filter.</div>';
      return;
    }
...
/** CSS-class-based visibility helpers (avoids inline style.display mutations) */
const showEl = el => { if (el) el.classList.remove('tab-hidden'); };
const hideEl = el => { if (el) el.classList.add('tab-hidden'); };
